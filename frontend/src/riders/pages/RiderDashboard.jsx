...
const RiderDashboard = () => {
  return (
    <div>
      ...
      <p className="mt-2 text-gray-600">Your personal hub for tracking buses and planning your journey.</p>
      ...
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          ...
          <div className="bg-white shadow rounded-lg p-6 h-96 flex flex-col justify-center items-center">
            ...
            <h2 className="mt-4 text-xl font-semibold text-gray-700">Live Bus Tracking</h2>
        ...
        <div className="space-y-8">
              ...
              {favoriteRoutes.map((route) => (
                <li key={route.id} className="py-3">
                  ...
                  <div className="flex justify-between items-center mt-1">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${route.status === 'On Time' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                      {route.status}
                    </span>
                    <span className="text-sm text-gray-600">Next bus: {route.nextBus}</span>
                  </div>
                </li>
              ))}
            </ul>
          ...
          <div className="bg-white shadow rounded-lg p-6">
            <h2 className="text-xl font-semibold text-gray-900 flex items-center">
              <BellAlertIcon className="h-6 w-6 mr-3 text-red-500" />
              Service Alerts
            </h2>
            <ul className="mt-4 space-y-3">
              {alerts.map((alert) => (
                <li key={alert.id} className="text-sm text-gray-700">
                  {alert.message}
                </li>
              ))}
            </ul>
